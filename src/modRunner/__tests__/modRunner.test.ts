...
describe("modRunner tests", () => {
  ...
  it("returns the right paths to run for mods", () => {
    const gottenPaths = getModsPaths(
      `${process.cwd()}/src/modRunner/__tests__/mocks/MockMods`,
      getModsPattern(true)
    );
  ...
  it("finds the right ts-config files", () => {
    const files = getTsConfigs(
      `${process.cwd()}/src/modRunner/__tests__/mocks/MockProject`
    );
